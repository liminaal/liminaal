// pages/index.js - 100% COMPLETO CON TODO TU HTML/CSS/JS + AUTH
...
export default function Home() {
  ...
  // TODO EL JAVASCRIPT ORIGINAL CONVERTIDO A REACT
  useEffect(() => {
    if (!user || !containerRef.current) return

    const container = containerRef.current

    // 1. SCREENSAVER IDLE TIMER (igual que tu original)
    let idleTimer
    ...
    // 2. GALLERY INTERACTIONS (igual que tu original)
    const boxes = container.querySelectorAll('.image-box')
    const hoverPreview = container.querySelector('#hoverPreview')
    const popup = container.querySelector('#popup')
    const popupCollab = container.querySelector('#popup-collab')
    ...
    boxes.forEach(box => {
      ...
      function positionPreview(x, y) {
        ...
        if (left + rect.width > window.innerWidth) left = x - rect.width - 18
        if (top + rect.height > window.innerHeight) top = y - rect.height - 18
        hoverPreview.style.transform = `translate3d(${left}px, ${top}px, 0) scale(1)`
      }

      box.addEventListener('mouseenter', e => {
        showTimer = setTimeout(() => {
          const previewThumb = container.querySelector('#previewThumb')
          const previewTitle = container.querySelector('#previewTitle')
          const previewDesc = container.querySelector('#previewDesc')

          previewThumb.src = box.dataset.img
      ...
      box.addEventListener('click', () => {
        ...
        const popupTitle = container.querySelector('#popup-title')
        const popupDesc = container.querySelector('#popup-desc')
    ...
    // 3. COLLABORATORS (igual que tu original)
    const collaborators = container.querySelectorAll('.collaborator')
    ...
    collaborators.forEach((coll, idx) => {
      ...
      coll.addEventListener('click', () => {
        const popupCollabImg = container.querySelector('#popup-collab-img')
        const popupCollabTitle = container.querySelector('#popup-collab-title')
        const popupCollabDesc = container.querySelector('#popup-collab-desc')

        popupCollabImg.src = coll.dataset.collImgFull || coll.dataset.collImg
        popupCollabTitle.textContent = coll.querySelector('.name').textContent
        popupCollabDesc.innerHTML = coll.dataset.collDesc.replace(/\n/g, "<br>")
        ...
        <div className="container login-container">
          <div className="login-box">
            ...
            <form action="/login" style={{marginBottom:"20px"}}>
              ...
              // ✅ USUARIO LOGUEADO - TU HTML COMPLETO + CSS COMPLETO
              return (
              <>
              <link href="https://fonts.googleapis.com/css2?family=Unbounded:wght@400;700&display=swap" rel="stylesheet"/>
              <style jsx global>{`
              /* TODO TU CSS ORIGINAL EXACTO */
              :root{--bg:#050505;--fg:#f5f5f5;--muted:rgba(245,245,245,0.12);--card-radius:12px;}
              *{box-sizing:border-box}
              html,body{height:100%;margin:0;background:var(--bg);color:var(--fg);font-family:'Unbounded',sans-serif;overflow-x:hidden}
              .container{max-width:1100px;margin:0 auto;padding:80px 20px;position:relative}
              h1{text-align:center;margin:0 0 18px;font-size:3rem}
              ...
              .hover-preview .preview-thumb img{width:100%;height:100%;object-fit:cover}
              ...
              .hover-preview .preview-meta p{margin:0;font-size:.85rem;color:rgba(245,245,245,.85)}
              ...
              .popup-content img{width:100%;height:auto;max-height:70vh;object-fit:contain}
              ...
              .collaborator img{width:100px;height:100px;object-fit:cover;border-radius:6px;margin-bottom:5px}
              ...
              .popup-collab .collab-left{flex:0 0 50%;min-width:220px;max-height:70vh;overflow:hidden;display:flex;align-items:center;justify-content:center}
              .popup-collab .collab-left img{width:100%;height:100%;object-fit:cover}
              ...
              @media (max-width:900px){.popup-collab .popup-inner{flex-direction:column}.popup-collab .collab-left{flex-basis:auto;height:45vw;max-height:55vh}}
              ...
              <iframe src="https://open.spotify.com/embed/playlist/5xzeVvVZU6mCfSwALg1yNp?utm_source=generator" width="100%" height="152" frameBorder="0" allowFullScreen allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"/>
              </div>
              ...
              <div className="collab-head"><h2>Collaborators</h2></div>
              ...
              <div className="name">Kaido</div><div className="role"></div>
              </div>
              ...
              <div className="name">Awitax</div><div className="role"></div>
              </div>
              </div>
              </div>
              </div>
              ...
              {/* POPUP COLLABORATOR */}
              <div className="popup-collab" id="popup-collab">
              <div className="popup-inner">
              <button className="popup-close" id="popup-collab-close">✕</button>
              ...
              <footer style={{textAlign:"center",padding:"2rem 0",color:"rgba(245,245,245,.6)",fontSize:".9rem"}}>© liminaal</footer>
              </>
              )
              }
